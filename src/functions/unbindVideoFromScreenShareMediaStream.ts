...
export default async (page: Page, state: State) => {
    ...
    await page.evaluate((DEBUG: boolean) => {
        ...
        if (videoTracks.length) {
            DEBUG && window.logger(`Unbinding ${videoTracks.length} video streams from \`window.mixedStream\``);
            videoTracks.forEach((videoTrack) => {
                window.mixedStream.removeTrack(videoTrack);
            });
        } else {
            DEBUG && window.logger('No video stream to remove from `window.mixedStream`');
        }
    }, DEBUG);
